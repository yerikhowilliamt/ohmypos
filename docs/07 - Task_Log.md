...
### TASK-134 — A demo-tenant seed that is safe to run against the production database

- **Date:** 2026-09-01
- **Module / Phase:** `apps/api/prisma/seed-demo.ts` — new script, no schema/migration/API change
- **Objective:** Give the business a showcase tenant it can seed straight into prod for prospect demos, without `prisma/seed.ts`'s development assumptions reaching a live database.
- **Relevant docs:** ADR-025 (tenant scoping, globally-unique `User.email`), ADR-024 (purchase-vs-stock unit), ADR-018 (report periods are Asia/Jakarta), ADR-014/ADR-015 (system refs), ADR-006 (UNPAID writes no LedgerEntry), ADR-013 (a recipeless product cannot be sold), Handbook §6 (synthetic data only), **ERR-051** (the blank chart this task surfaced).
- **What was done:** New `prisma/seed-demo.ts` (~1.150 lines) plus a `db:seed:demo` script in `apps/api/package.json`. Provisions one tenant — Kopi Senja, 2 branches, 4 accounts, 7 categories, 4 users, 11 raw materials, 8 products with recipes, 3 suppliers, 9 purchases (3 bulk + 6 weekly restocks, one UNPAID with a partial settlement), ~350 sales over 43 days, 6 manual ledger entries, 8 bank transactions with allocations covering all four `TransactionStatus` values, opening stock, 2 devices, attendance and 3 leave requests. Every derived figure goes through `SupplierPurchasesService` / `PayablesService` / `SalesService`; only the inputs are literals. Config is entirely env-driven with no defaults, and the script prints a plan and exits unless `--yes` is passed.
- **Decisions made during this task:** (1) A **separate file**, not a parameterised `seed.ts` — that file upserts the tenant with slug `default`, creates a `PlatformAdmin` and two staff users on the hardcoded password `ChangeMe123!`, and pins four Accounts to hardcoded primary keys `00000000-…-0001..0004`, which a second tenant collides with outright. Merging the two would put the development fixture one env var away from the real business's books. (2) Every id is a deterministic UUIDv5 derived from the tenant slug (`uuidFrom`), and the sale mix comes from a slug-seeded mulberry32 PRNG, so a re-run replays through the services' `idempotencyKey` path instead of duplicating. (3) The sale plan is simulated against the **literal purchase quantities**, never against live `currentStock` — reading live stock would produce a different (depleted) plan on the second run, so a sale skipped once would be created later under a fresh key and the demo's revenue would climb on every re-run. (4) `DEMO_VOLUME` scales restock quantities and daily sale count together and is folded into the purchase and sale idempotency keys, because raising it must record a new, larger restock rather than replay the old one. (5) Trading hours are generated as **Asia/Jakarta wall-clock times** and converted back to instants (`atWib`). The first version used `setUTCHours(8..20)`, which is the same bug in two places: the demo's opening hours rendered as 15:00-03:00 WIB on screen, and each day's takings split across two report days — every report range in this product is pinned to `z.literal('Asia/Jakarta')` (ADR-018, `report.schema.ts:40`). (6) Perishables are restocked **weekly across the window**, not once up front, and the simulator's safety floor is an **absolute** quantity (60% of `lowStockThreshold`) rather than a share of cumulative purchases. Together these were one bug: a share-based floor rises with every delivery, so the simulated stock ran out toward the end and the sale rate decayed from 3.5/day to 2.3/day — meaning the dashboard's default "hari ini" view opened on the thinnest day in the dataset. (7) Opening stock is written **directly**, not through `OpeningStockService.upsert` — that method applies the declaration as a delta `OPENING` StockMovement, which would double-count a figure this seed derives from the movements it just created. (8) Every product carries a recipe: `SalesService` rejects a recipeless one (ADR-013), so `seed.ts`'s `Air Mineral` pattern would put an unsellable item on a demo menu. (9) **A cash deposit is not income, and monthly costs do not share a date.** The first draft booked "Setoran tunai kasir ke bank" as a 3.000.000 INFLOW under `Penjualan` — a transfer between two Accounts, behind which the sales had already written their own INFLOW entries, so it double-counted revenue and spiked one day to 9x the ~390.000 median. It also booked payroll and rent to the same date, giving one -5.279.035 day. The dashboard plots `income` and `netProfit` on ONE shared Y axis, so either outlier alone flattens every other day to a straight line. Removing the deposit and splitting rent (start of month) from payroll (two fortnightly runs) cut the axis span from 8.742.035 to 2.806.955.
- **Status:** Done
- **Handoff notes:** Verified end-to-end against local Postgres, not only typechecked. Two consecutive runs produce byte-identical counts, revenue (`17.822.000,00`) and stock — 352 sales, 632 sale items, 1.770 stock movements, 367 ledger entries, 9 purchases. Trading hours land at 08:00-21:59 WIB and the daily rate is flat across the window (7,6/day over the first week vs 8,4/day over the last), so the dashboard's default day view is representative rather than starved. All four `TransactionStatus` values are present (5 UNRESOLVED, 1 PENDING_REVIEW, 1 PARTIALLY_ALLOCATED, 1 MATCHED), the payable lands `PARTIALLY_SETTLED` with `sisa=294.000,00`, and the latest-cost write-back reproduces each material's intended `unitCost` exactly. Five guards were each exercised and each aborted before writing anything: slug `default`, an owner email already owned by another tenant, a short password, a missing env var, and an out-of-range `DEMO_VOLUME` — plus the important one, a target tenant that already contains a user outside the derived demo accounts, which is what keeps a slug typo from injecting a fictional coffee shop into a real customer's books. One product defect fell out of this work and is logged separately as **ERR-051**: `ReportLineChart` passed `dot={false}`, so a single-point series rendered as a blank panel — which the dashboard produces on the 1st of every month, since its range is "start of month → today". Fixed in two layers — `lineDotFor()` in `ReportChart.tsx` so a deliberately one-day range on the Reports page still shows its value, and a rolling 30-day window (`trendWindowRange`) for the dashboard's trend panel so it always has a shape; the "Total bulan ini" figure stays month-to-date via `profitLoss.totalIncome`, which the dashboard already fetched, so the wider chart costs no extra request. That fix is product code, not seed code, and applies to every tenant. **Three things to know before running it on prod:** the script never writes a `PlatformAdmin` (that stays `scripts/create-platform-admin.ts`) and never touches slug `default`; the demo passwords come from `DEMO_OWNER_PASSWORD` / `DEMO_STAFF_PASSWORD` and are the only credentials it will ever accept, so they must be set deliberately rather than inherited; and there is **no un-seed** — removing a demo tenant means deleting its rows by hand across ~22 tables, so choose the slug once.

### TASK-133 — BCA e-statement PDF import, with a synthetic sample generator built from the real layout

- **Date:** 2026-09-01
- **Module / Phase:** `apps/api/src/modules/import` — second PDF issuer, extending TASK-1xx / ADR-022's Mandiri-only PDF support
- **Objective:** Accept the BCA "Laporan Mutasi Rekening" PDF e-statement in the Rekonsiliasi → Impor flow, and generate a synthetic sample set for it matching the one that already exists for Mandiri.
- **Relevant docs:** ADR-026 (written by this task), ADR-022 (extended, not superseded), ADR-010 (contracts as source of truth), PRD §5.7, `docs/e-statements/README.md`.
- **What was done:** New `parsers/bca-pdf.parser.ts` with `extractBcaPeriod` / `extractBcaRows` / `parseBcaRows` and the `BcaPdfParser` strategy. `BCA_PDF` added to `BankImportFormatSchema` and `BANK_IMPORT_FORMATS` (label `BCA (PDF e-Statement)`, container `pdf`), plus the `BankParserFactory` case, the controller's `ApiQuery` enum and the unsupported-format message. No schema, migration, route or frontend-component change — the format picker is data-driven, so the option appeared with no UI edit. New `docs/e-statements/gen-bca-pdf.js` + `make-bca-statements.js` generating six synthetic statements (normal / multipage / duplicates / edge cases / empty / year-boundary), README rewritten to cover both issuers, and `.gitignore` made default-deny for PDFs in `docs/e-statements/` with the synthetic samples allow-listed. Tests: 32 unit (`bca-pdf.parser.spec.ts`, over hand-built text geometry) + 6 end-to-end over the generated PDFs (`bca-pdf.samples.spec.ts`) + 1 new frontend test asserting `BCA_PDF` routes to `/import/pdf` and not `/import/csv`.
- **Decisions made during this task:** All the consequential ones are in **ADR-026**. Two worth repeating here because they were found empirically, not designed: (1) a row must run to the **next** date marker with no height cap — the first draft reused Mandiri's `MAX_ROW_HEIGHT` on every row and silently truncated the description of any row taller than 60pt, which the real statement never triggers because its tallest row is 48.5pt; (2) the closing totals block has to be matched **explicitly** rather than out-capped, because it sits ~68pt below a three-line last row but only ~44pt below a single-line one, and it right-aligns its amounts into the CBG column and its counts into MUTASI. Both defects were caught by the sample generator, not by the real file. Smaller: the description joins KETERANGAN + detail + CBG, keeping the branch code because it is the only field separating some same-day fee rows; and the 500-character truncation guard is effectively unreachable on a real BCA statement (detail lines are ≤18 characters and rows hold ≤5 of them), so the fixture that exercises it uses a deliberately 12-line row.
- **Status:** Done
- **Handoff notes:** Verified against the real 7-page / 63-transaction August statement: every row parsed, and the CR/DB totals (20 / 3,440,700.00 and 43 / 3,826,360.00) reconcile **exactly** against the statement's own `MUTASI CR` / `MUTASI DB` summary block. That file is **not** committed and must not be — it carries a live account number, the holder's name, a phone number and counterparty names; `.gitignore` now blocks it by pattern. Three things the next issuer's parser should know: **do not** try to generalise the two existing parsers behind a shared column-map config (see ADR-026's rejected alternatives — they share the idea of a table and nothing else); the year-derivation rule is BCA-specific and drops any row whose month is outside the statement period except a December-on-January carryover, so a genuinely out-of-period row surfaces as a silent unreconciled gap rather than an error; and `ADR-022` still describes `pdf-parse` as the extraction library while the code has since moved to `pdfjs-dist` (`pdf-text.util.ts`) — that ADR is stale on that point and was left alone as out of scope here, but it should be corrected.

### TASK-132 — A suspended tenant's users reach the app and are told why, instead of looping on the login screen

- **Date:** 2026-08-31
- **Module / Phase:** v2 platform console fallout — `TenantStatusGuard`, `auth`, and the tenant app shell
- **Objective:** Let the OWNER of a suspended tenant get as far as the dashboard and meet a modal saying the business is suspended and to contact an OhMyPos admin — instead of what actually happened, which was an endless bounce back to `/login`.
- **Relevant docs:** ADR-025 (Decision 8 impersonation exemption), ADR-011 §3, System Design v4 §5, Playbook §8, DESIGN.md (dialog), **ERR-050** (the loop this task uncovered).
- **What was done:** `TenantStatusGuard` gained a second exemption, `GET /auth/me`, matched on method and path together so `PATCH /auth/me` stays blocked; `isLogout` became `isExempt`. `AuthService.getProfile` returns the new `SessionResponseSchema` (`UserResponse` + `tenantStatus`), reading the tenant row through the unscoped client. `apps/web/lib/session.ts` types `getSession`/`requireRole` as `SessionResponse`. `AppShell` renders `SuspendedTenantDialog` and withholds `children` when suspended. `packages/ui` `DialogContent` gained `showCloseButton` (default `true`, so every existing dialog is unchanged). Six component tests, three e2e assertions.
- **Decisions made during this task:** (1) **Only `/auth/me` was opened, not all GETs.** Presented with the read-only-mode option, the user chose the narrow carve-out: `/auth/me` returns the caller's own identity and no business data, so exempting it reveals nothing they did not supply by logging in — where opening every GET would leave a suspended tenant usable for reading reports, which is precisely what ADR-025 says suspension prevents. (2) The shell renders but `children` does not. `children` is server-rendered output whose client components fetch on MOUNT, so leaving them out of the tree is what stops a dozen 403s stacking up behind a modal that already gives the one reason for all of them. (3) The modal cannot be dismissed — no X, no Escape, no click-outside — because nothing behind it works; the single action is logout, which the guard still permits. (4) **An impersonating operator is excluded from the modal**, or the frontend would silently undo ADR-025 Decision 8: reading a suspended tenant's books to find out why it was suspended is the main reason that exemption exists. (5) `tenantStatus` lives on `/auth/me` alone; `PATCH /users/:id` and the rest answer questions about a person, and this field is about the business. (6) A missing tenant row reports `SUSPENDED`, not `ACTIVE` — that session is broken either way, and `ACTIVE` is the one answer that would hide it.
- **Status:** Done
- **Handoff notes:** `turbo run lint typecheck test build` green (15/15); full API e2e 539/539 across 22 suites; web 561/561. **The guard's exemption list is the security-sensitive part of this change** — it is matched on method AND path, and the `PATCH /auth/me` → 403 e2e assertion exists specifically to catch a future edit that loosens it to path-only. Under the full parallel `turbo` graph, `components/reconciliation/BankStatementImportCard.test.tsx > surfaces a failed import without clearing the form` failed once on a loaded machine and passed on every isolated and repeat run; the file is untouched by this task, so it is being recorded as a load-related flake rather than a regression — worth a look if it recurs. Not built: no banner or countdown for a *scheduled* suspension, no self-service route to request reinstatement, and KASIR/ADMIN of a suspended tenant see the same modal as the OWNER (deliberate — the message names the business, not the role).

### TASK-131 — Platform operator can correct a tenant OWNER's login email

- **Date:** 2026-08-31
- **Module / Phase:** v2 platform console (`modules/platform`) — recovery paths, continuing TASK-130
- **Objective:** Give a platform operator a way to fix the email a tenant OWNER logs in with, for the case that motivated it: the address was mistyped at provisioning, so nobody in that business can log in at all and nothing inside the tenant can repair it.
- **Relevant docs:** ADR-025 (Decision 5 platform admin, Decision 6 globally-unique `users.email`, Decision 7 provisioning, Decision 8 read-only impersonation), ADR-011 §2, Playbook §4 (contracts), §8 (guards), §9 (never echo a credential), AGENTS.md (approval gate).
- **What was done:** `PATCH /platform/tenants/:id/owner-email` on `PlatformTenantsController` — the same class as the password reset, so it inherits `PlatformAuthGuard` rather than depending on somebody remembering it (DEBT-066), throttled 5/60s to match. New contract `platform-owner-email.schema.ts` (`userId`, `newEmail`, `reason` ≥10 chars, `acknowledgeExistingData`). `TenantDetailResponse` gained `isPristine`. `PlatformTenantsService.updateOwnerEmail` verifies the named user really is an OWNER of that tenant (one 404 message for all three failure shapes, as `resetOwnerPassword` does), rejects a taken address with `OwnerEmailTakenException` (checked up front *and* on P2002), then writes the new email, nulls `refreshTokenHash` and bumps `tokenValidFrom`. New pure helper `tenant-data-check.ts` + spec. Frontend: `useUpdateTenantOwnerEmail` and a third dialog on the tenant detail page, with the confirmation checkbox rendered only when `isPristine` is false.
- **Decisions made during this task:** (1) **The user chose a warning over a hard gate.** The request as stated was "only when the tenant has no data at all"; presented with three options, the user chose to allow the change at any time but to surface the tenant's state — so `acknowledgeExistingData` is a speed bump, enforced server-side (409 `TenantHasDataUnacknowledgedException`), not a wall. The checkbox is real, not decoration: the API refuses without it. (2) "Pristine" excludes everything provisioning itself creates — baseline of 1 for `users`, and only `isSystem: false` branches are counted — which is why the decision lives in a pure function with its own spec; the arithmetic is trivial, the baseline is the part that can be got wrong. (3) The refusal message names what was found ("1 supplier"), because an operator cannot judge whether to acknowledge a warning that will not say what it saw. (4) PATCH rather than POST: this edits one field, where `reset-owner-password` performs an action. (5) Sessions are revoked even though the locked-out owner has none — the old address may have been deliverable by someone else. (6) The password is deliberately untouched; an e2e assertion holds that line, so the two recovery endpoints cannot quietly merge. (7) No schema change and no migration, so the AGENTS.md database gate was never reached.
- **Status:** Done
- **Handoff notes:** `turbo run lint typecheck test build` green (15/15); platform e2e 34/34 (3 new). **Throttle budget is a live constraint in `platform.e2e-spec.ts`:** the route is 5/60s and the route-protection block spends two of those probing every `/platform/*` path on every run, so the new describe block has exactly three requests left — a fourth turns the last assertion into a 429. The block also runs **last** on purpose: it rewrites `plat-kopi`'s owner email, and the impersonation suite above asserts on that address. `deleteProvisionedTenants` now also clears suppliers, because the new suite writes one and every FK in this schema is `Restrict` — without that, a run that died mid-suite would break the *next* run's tenant cleanup. Not built, and deliberately: no change to the owner's *name*, no equivalent for KASIR/ADMIN emails (an OWNER can already fix those from inside via `PATCH /users/:id`), and no audit table — like the password reset, the trail is a `logger.warn` line carrying both addresses, the pristine verdict, and the reason.
...
- **Date:** 2026-08-30
- **Module / Phase:** Defect remediation (plan `docs/plannings/2026-08-30-remediasi-full-test-sweep.md`, findings from `docs/testing/2026-08-30 - Full_Test_Sweep.md` §3)
- **Objective:** Close the five defects the sweep measured, each with a test that would have caught it — and, where a fix needs a schema change, stop at the approval gate with numbers rather than opinions.
- **Relevant docs:** AGENTS.md §AI Gatekeeper (schema gate), §Strict Scope, §Contributing 7 and 10; ADR-010 (contracts are the single source of truth); sweep §3 and §5.
- **Items closed — cross-checked against the diff:**
  - **D-5 (ERR-049)** — `shm_size: 1gb` on the `postgres` service in `docker-compose.yml`, with the measured justification in a comment. Verified: `df -h /dev/shm` inside the container reports 1.0G, was 64M. Done first, because until it was, every D-1 measurement was contaminated by SQLSTATE 53100.
  - **D-4 (ERR-048)** — `BusinessProfileClient.tsx:276`, `<Card bg-surface-muted>` → `<Card className="bg-surface-muted">`.
  - **D-2 (ERR-046)** — `PostgresTriggerExceptionFilter` answers **413** for `entity.too.large` and logs it at **warn**, ahead of the 500 branch. New `postgres-trigger-exception.filter.spec.ts` (2 tests) pins both branches. Verified live with `curl` against the running API: 500 before, 413 after.
  - **D-3 (ERR-047)** — `LeaveRequestListQuerySchema` now overrides `limit` to `.max(500)`, matching `AttendanceQuerySchema`; `AttendanceCalendarMatrix.tsx` gains `omittedLeaveCount` and its on-screen truncation warning, mirroring the attendance side. Two new tests parse the outgoing params through the **real Zod schemas** instead of a mock. Verified live: the same request that returned 400 now returns 200.
  - **D-1a (ERR-045)** — `INVENTORY_SUMMARY_TX_OPTIONS = { maxWait: 5_000, timeout: 20_000 }` passed to `$transaction`; the callback body is untouched, because the one-instant consistency it provides is the point. New `inventory-summary.service.spec.ts` (2 tests) pins both the explicit timeout and the three-reads-in-one-transaction shape.
- **NOT closed: D-1b (the covering index).** It touches `schema.prisma` and needs a migration, so it stopped at the approval gate, unapproved and unwritten. No index change is in this diff. It is recorded here as open work, not as done — the ERR-029 failure mode.
- **What the measurement found, and why it changes the D-1b proposal.** `EXPLAIN (ANALYZE, BUFFERS)` on `ohmypos_volume` (1,808,816 movements, 838 MB) for the period `groupBy`: **Parallel Seq Scan, 1235 ms cold / 343 ms warm, 107,244 shared blocks read**, with 591,959 rows discarded by the filter per worker. Three candidates were then built and measured on that database, and dropped again afterwards:
  - The plan's proposal — widening the existing index to `(tenant_id, raw_material_id, movement_date, direction, reference_type)` — **the planner ignores entirely.** Still a Parallel Seq Scan. `raw_material_id` sits second with no equality predicate against it, so a range on `movement_date` cannot be driven through it; adding a fifth column changes nothing about that.
  - `(tenant_id, movement_date, raw_material_id, direction, reference_type) INCLUDE (quantity)` → **Index Only Scan, 15.6 ms, 527 blocks.** 79× faster, 200× fewer blocks. 228 MB.
  - The same index without `INCLUDE (quantity)` → Index Scan, 19.3 ms, 155 MB. `quantity` is what the `SUM` needs, so without it the scan is not index-only; at this row count the heap fetches cost ~4 ms and 73 MB of index is saved.
- **Post-fix load measurement (D-1, run after D-1a and D-5 landed).** A production build on :4098 against `ohmypos_volume`, `THROTTLE_LIMIT` raised so the measurement reaches the database rather than the rate limiter, 10 serial requests then 20-second soaks at 1 / 10 / 25 / 50 concurrent clients on `GET /inventory/summary` — the sweep's own harness shape:

  | Concurrency | rps | p50 | p95 | p99 | errors — before (sweep) | errors — after |
  |---|---|---|---|---|---|---|
  | 1 | 3.1 | 323 ms | 340 ms | 402 ms | 0 | **0** |
  | 10 | 5.3 | 1589 ms | 3304 ms | 3475 ms | 0 | **0** |
  | 25 | 6.0 | 4539 ms | 6500 ms | 6769 ms | 22 | **0** |
  | 50 | 9.2 | 6626 ms | 9115 ms | 9969 ms | 18 | 48 |

  **The 25-client failure is gone, and the failure mode at 50 has changed.** Not one `A batch query cannot be executed on an expired transaction` occurred — the mid-flight expiry ERR-045 described is eliminated. All 48 remaining errors are `Unable to start a transaction in the given time`: the new `maxWait: 5_000` refusing to queue for a connection that is not free. That is the bound doing its job — failing fast in 5 s instead of holding a client for 20 and then dying — but it is still a 500 to the caller, and it means at 50 concurrent clients the connection pool, not the timeout, is the limit. **Also confirmed: zero SQLSTATE 53100 across the entire run, against 103 in the sweep.** D-5 holds under load.
- **What the D-1b index is actually worth end-to-end.** The same soak with the candidate index in place (built on `ohmypos_volume`, then dropped): c50 errors **48 → 10**, p50 at c1 323 → 238 ms, at c25 4539 → 3489 ms, throughput +20-30% at every level. Real, but nowhere near the 79× that `EXPLAIN` on the single query promised — **because the index only fixes one of the endpoint's three reads.** With it in place the period `groupBy` drops to a 14 ms Index Only Scan, and the *prior-bucket* `groupBy` (`movement_date < periodStart`) becomes the dominant cost: it aggregates all history before the period, so it reads ~99% of the table and remains a Parallel Seq Scan that no index can help. The endpoint's floor is therefore architectural — ADR-008's compute-at-query-time over unbounded history — not a missing index. D-1b is worth doing and will not, on its own, make this endpoint fast.
- **Decisions made during this task:** (1) **Measure before proposing a schema change.** The plan's index was a reasonable inference from the schema and was wrong — asking for approval on it would have been asking for approval for a guess, and it would have added write cost to `stock_movements` (touched by every sale) for no read benefit. (2) **Raising the leave-request cap alone was rejected as insufficient**: it moves the silent-truncation cliff from 100 to 500 rather than removing it, so the truncation banner is part of the fix, not an extra. (3) **413 handled inside the existing catch-all filter, not a second `@Catch(PayloadTooLargeError)`**: Nest resolves filters by registration order against a catch-all, and getting that order wrong fails silently. (4) Two spec files needed small deviations from the plan's literal code to satisfy the repo's own lint rules (`no-unsafe-member-access`, `no-unnecessary-type-assertion`, unused args); the assertions are unchanged.
- **Status:** Done for D-1a, D-2, D-3, D-4, D-5. **Open for D-1b**, at the approval gate.
- **Handoff notes:** `pnpm exec turbo run lint typecheck test build --force` — lint, typecheck, build and the API suite green with no cache. **232 API unit** (was 228, +4), **555 web unit** (was 553, +2), **519 API e2e** (unchanged), **16 Playwright** (unchanged) — every baseline from the sweep held. **One honest caveat:** the web unit suite is green 555/555 standalone (verified four times) but **flakes under the fully-parallel `--force` run** — a *different* set of tests failed on each of three attempts (`RecipeEditorDialog`, then five across `StockMovementsClient.export` / `PurchaseEntryFormDialog` / `AttendanceLogTable.search` / `SalesHistoryClient.search`). All of them are real-timer `waitFor` assertions over `useDebouncedValue`, none is in a file this task touched, and the set is not reproducible — the mechanism is CPU contention starving a debounce window while the API suite and both builds run alongside. Not a regression from this task, but it does mean the combined gate is not a trustworthy signal on this machine; run `pnpm --filter web test` on its own to judge the web suite. The D-3 contract test was confirmed to go **red** when the cap was temporarily reverted to 100, then restored; a test never seen failing proves nothing. Scratch indexes built on `ohmypos_volume` for the measurement were dropped — that database's index list is back to its original six. New debt: **DEBT-069** (production `/dev/shm` on Render is unverified and unfixable from this repo), **DEBT-070** (no route for a non-transactional migration — this is the blocker D-1b would hit in production), **DEBT-071** (`react/no-unknown-property` not enabled), **DEBT-072** (`volume-smoke.e2e-spec.ts` does not run at volume), **DEBT-073** (no sustained load test — the reason three of these five defects could only be found by hand). Sweep §5's third coverage gap, "frontend tests cannot catch contract drift", is closed by the two schema-parse tests. Deliberately untouched: `/reports/daily` at 5.7 s and `/inventory` at 4.3 s, both undiagnosed.

### TASK-128 — v2 Phase 6: the tenant-isolation suite, the platform browser boundary, and a blocker it found

- **Date:** 2026-08-30
- **Module / Phase:** v2 Phase 6 — tests and documentation (plan `docs/plannings-v2/2026-08-29-multi-tenant-foundation.md` §Fase 6)
- **Objective:** Prove from the outside that two tenants cannot see each other — across every list, detail, write and report endpoint — and that the two audiences cannot walk into each other's app in a real browser.
- **Relevant docs:** ADR-025 Decision 3(c) (three layers of enforcement), Decision 6 (email stays global), ADR-014 (`isSystem`), plan §6.1–§6.3, DEBT-064.
- **What was done:** `apps/api/test/tenant-isolation.e2e-spec.ts` — 37 tests over two tenants whose fixtures are **deliberately named identically**. `apps/web/e2e/platform-boundary.spec.ts` — 7 Playwright tests, plus a platform storage state in `auth.setup.ts`/`fixtures.ts`. `platform.e2e-spec.ts` strengthened so the e2e tenant owns a system branch before a tenant is provisioned through the API. Migration `20260830090000_scope_branch_flags_to_tenant` (approved first) fixing ERR-044.
- **The suite found a total blocker that had been live since Phase 1.** `branches_single_system` and `branches_single_main_store` — partial unique indexes from TASK-120 — were **globally** scoped, so only one tenant on the entire platform could own a system branch or a main store, and the second tenant ever created would fail at `ensureSystemRefs` with the whole provisioning transaction rolled back. Neither index is in `schema.prisma` (Prisma cannot express a partial index), so the Phase 1 constraint audit, which read the schema, never saw them. Full account in **ERR-044**; the general gap is **DEBT-068**.
- **Decisions made during this task:** (1) **Every assertion is on IDs, never names, and every fixture shares a name with its twin.** A leak that returns "Cabang Utama" when the other tenant also has a "Cabang Utama" is invisible to a name assertion. The duplicate names simultaneously prove the eight uniques from plan §1.3 really did become composite — if they had not, the second tenant would not insert and the suite would fail in `beforeAll`. (2) **Detail endpoints must answer 404, never 403**, and the suite asserts the number: a 403 confirms the row exists, which leaks the id space one bit at a time. (3) **The two tenants' ledger amounts differ (100.000 vs 250.000)** so a missing `tenant_id` predicate in a report shows up as a wrong FIGURE, not as an extra row nobody inspects — leaking would make both sides 350.000. (4) **One test bypasses the client extension entirely** and writes through `UnscopedPrismaService` to prove the composite FKs reject a cross-tenant reference on their own — that is the layer that survives a service-layer mistake. (5) **A "not empty" assertion sits beside the disjointness ones**, because a filter so tight it returns nothing would pass every disjointness check. (6) The index fix is on `("tenant_id") WHERE is_system`, not `(tenant_id, is_system)`: the `WHERE` already restricts the index to rows where the flag is true, so the flag carries no information inside it.
- **Status:** Done
- **Handoff notes:** `pnpm turbo run lint typecheck test build` green (15/15); **519 e2e across 22 API suites**; Playwright `platform-boundary.spec.ts` 11/11 green against the running dev stack. Migration applied to `ohmypos_db` and `ohmypos_e2e`, both verified to have zero violating rows first. **What is still NOT covered, and should not be mistaken for covered:** `product-profit` and `top-products` are the two report endpoints the isolation suite does not assert — both need a real Sale (recipe, stock, an HPP snapshot) rather than a bare ledger entry. They go through the same `saleScope()` fragment as the four that are asserted, but that is an argument, not a test; noted on DEBT-064. Also uncovered: the POS sale flow end-to-end under two tenants, and the Playwright suite does not exercise impersonation (it needs a tenant provisioned mid-test). Plan §6.1's `platform-auth.e2e-spec.ts` and `impersonation.e2e-spec.ts` were not created as separate files — both were already covered by `platform.e2e-spec.ts` in TASK-126, including the router-enumerating 401 sweep, and a second copy would drift rather than add.

### TASK-127 — v2 Phase 5: the `/platform` console in `apps/web`

- **Date:** 2026-08-30
- **Module / Phase:** v2 Phase 5 — platform route group, session helper, shell, and the impersonation hand-off (plan `docs/plannings-v2/2026-08-29-multi-tenant-foundation.md` §Fase 5)
- **Objective:** Give the platform operator a console: sign in with the platform key pair, see cross-tenant totals, provision and suspend tenants, and read a tenant's books through a read-only impersonation session that is impossible to mistake for an ordinary one.
- **Relevant docs:** ADR-025 (Decisions 5, 7, 8), System Design v4 §5 (frontend gating is UX, never enforcement), DESIGN.md §10 (shell), Playbook §5 (money as a string), plan §5.1–§5.2.
- **What was done:** `lib/platform-session.ts` (`getPlatformSession`, `requirePlatformAdmin`), `lib/impersonation.ts` (cookie names + server-side read), `lib/nav-config.ts` gained `PLATFORM_NAV_ITEMS` as a separate constant. `components/platform/`: `PlatformShell`, `ImpersonationBanner`, `TenantStatusBadge`. Routes: `app/platform/login/page.tsx` and an `app/platform/(console)/` group holding the overview, the tenant list (with the create-tenant dialog) and the tenant detail page (stats, suspend/reactivate, impersonate, and the impersonation audit trail). Hooks `usePlatformTenants.ts` / `usePlatformMetrics.ts`. `app/api/platform/impersonation/route.ts` installs and clears the impersonation cookies. `proxy.ts` gates `/platform/**` on the platform cookie. `AppShell` takes an optional `impersonatedLabel` and all three tenant layouts pass it. Tests: `proxy.test.ts` (12, new file), `ImpersonationBanner.test.tsx` (4), plus additions to `lib/nav-config.test.ts` (3) and `lib/api.test.ts` (3).
- **Decisions made during this task:** (1) **The console gets its own `PlatformShell` instead of reusing `AppShell`** — see the section below. (2) **The impersonation token reaches the browser through a Next route handler**, because the API returns it in a response body (it is a tenant token minted by a platform route) and the `access_token` cookie is HttpOnly, so client JS cannot install it. (3) **A second, non-HttpOnly `ohmypos_impersonation` cookie carries the banner label.** It is a UI hint and forgeable by design; the read-only enforcement is `ImpersonationReadOnlyGuard` keyed on the signed `imp` claim. The alternative — decoding the JWT in the web app — would mean shipping the tenant signing secret to the frontend for a piece of chrome. (4) **The banner is undismissable**, and follows an impersonated OWNER into the POS layout too: a read-only till that silently refuses every sale is worse than one that says why. (5) **The tenant-status toggle and the impersonate button live on the detail page beside the audit trail**, so nobody reaches the button that adds to that record without passing the record.
- **Two plan corrections, one of them structural.** (a) **§5.1's `app/(platform)/` cannot work as written.** A bare route group adds no URL segment, so `app/(platform)/page.tsx` resolves to `/` — colliding with the existing `app/page.tsx`, which is a build error — and `app/(platform)/tenants/` to `/tenants`, which §5.2's own proxy rule ("path berawalan `/platform`") would then never match. The group is nested under a literal segment instead: `app/platform/(console)/`, with `app/platform/login/` outside it so the login page is not wrapped by the layout that redirects unauthenticated visitors to the login page. (b) **`apiFetch` had to learn about the second audience.** Its refresh-on-401 sent every 401 to `/auth/refresh` and, on failure, to `/login`. A platform 401 would therefore have rotated a tenant session the operator does not hold and dumped them on the shop owner's login page with their console session still valid. It now picks the refresh endpoint and the login href by path prefix, and dedupes per audience rather than globally.
- **Why `PlatformShell` rather than `AppShell`:** `AppShell` is typed on `UserResponse` and threads `user.role` into `Sidebar` (nav), `Topbar` (breadcrumbs) and `SidebarAccountCard` (avatar, role label); it also carries a dark-mode toggle, nav search, collapsible groups and a `/profile` link. A platform admin has no role, no branch, no photo and no profile page. Reuse meant either passing a counterfeit `UserResponse` — which renders an OWNER's tenant nav, links and all — or widening four components every existing tenant route depends on, for a console with two flat destinations. `PlatformShell` composes the same `packages/ui` sidebar primitives and the same tokens, so the two look identical without the tenant app carrying the risk. No new primitives were added, which is what plan §5's "pakai ulang `packages/ui` sepenuhnya" actually asks for.
- **Status:** Done
- **Handoff notes:** `pnpm turbo run lint typecheck test build` green (15/15); 73 web test files, 550 → 560 tests. `next build` confirms the routes resolve as `/platform`, `/platform/login`, `/platform/tenants`, `/platform/tenants/[id]`. **Not done, and Fase 6's job:** no Playwright coverage — `e2e/auth.setup.ts` still has no platform storage state, and `role-boundary.spec.ts` does not yet assert that a tenant role cannot open `/platform`. The vitest suite covers the proxy decision table and the banner, which is the part most likely to be got wrong by hand, but nothing yet drives the console end to end in a browser. Two smaller things: the console has **no tenant search or filter** (the list is paginated only — fine at today's tenant count, wrong at a hundred), and **`ImpersonationBanner` shows the acting-as email rather than the tenant name**, because that is what `ImpersonationSessionResponse` carries; showing the business name would mean adding it to that response.

### TASK-126 — v2 Phase 4: the `/platform/*` API, tenant provisioning, and read-only impersonation

- **Date:** 2026-08-30
- **Module / Phase:** v2 Phase 4 — `apps/api/src/modules/platform/` and four new contract files (plan `docs/plannings-v2/2026-08-29-multi-tenant-foundation.md` §Fase 4)
- **Objective:** Give the platform operator an API to see, provision, suspend and look inside tenants — without any of it being able to write through an impersonated session, and without a provisioned tenant being broken from birth.
- **Relevant docs:** ADR-025 (Decisions 5, 7, 8, §4.1–§4.3), ADR-011 §2 (an OWNER has no branch), ADR-014 / `system-refs.ts` (the rows a tenant cannot trade without), ADR-008 (compute at query time), Playbook §5 (money as a fixed-scale string).
- **What was done:** Four contract files — `tenant.schema.ts` (status, slug rule, create-with-owner, list/detail responses), `platform-admin.schema.ts`, `platform-metrics.schema.ts`, `impersonation.schema.ts` — registered in the barrel after `business-profile.schema`. `PlatformModule` with `PlatformAuthController` (`login`/`refresh`/`logout`/`me`) and `PlatformTenantsController` (metrics overview, tenant list/create/detail/patch, the impersonation audit trail, start and end impersonation), backed by `PlatformAuthService`, `PlatformTenantsService`, `PlatformMetricsService` and `ImpersonationService`. New `@ReqPlatformAdmin()` decorator. New `ImpersonationReadOnlyGuard` registered in `APP_GUARD` between `JwtAuthGuard` and `TenantStatusGuard`, plus an optional `imp` claim on `JwtPayload`. New `platform.e2e-spec.ts` — 22 tests.
- **Decisions made during this task:** (1) **`POST /platform/tenants` provisions through the tenant-FILTERED client inside `runWithTenant`, with the tenant id generated in application code.** Seeding through the unscoped client would let `ensureSystemRefs` find ANOTHER tenant's `isSystem` branch and skip creating one — a tenant that looks provisioned and silently attributes its central ledger entries to a different business. The id cannot come from `@default(uuid())` because the scope must be open before the first scoped write. (2) **`ImpersonationReadOnlyGuard` is global, not per-route.** The impersonated token is presented to ordinary tenant endpoints, so a per-endpoint opt-in would need all ~120 of them to remember, and the one that forgot would be the writable hole. (3) **An impersonation session is EXEMPT from `TenantStatusGuard`** — the most likely reason to open a tenant's books is to work out why it was suspended, and a suspended tenant cannot log in to show anyone. Safe only because the read-only guard runs first, which is why the two are ordered that way in `APP_GUARD`. (4) **Platform token lifetimes are derived from the cookie max-ages** rather than read from env, so a cookie cannot outlive its token — that combination produces a session that looks live and 401s on every call. (5) **There is no `DELETE /platform/tenants/:id`.** Every FK in this schema is `Restrict` precisely because deleting a tenant means deleting its financial history; `status: SUSPENDED` is the off switch. (6) **`ImpersonationService.end` closes only the caller's own open session**, so one operator cannot blank an audit record another is still writing. (7) `grossRevenue` everywhere excludes `VOIDED` sales, so the platform figures agree with what each tenant sees in its own reports (DEBT-010).
- **The route-protection test enumerates from module metadata, not from a list.** `platform.e2e-spec.ts` reads `@Module({ controllers })` off `PlatformModule` and then the `@Controller`/`@Get`/`@Post` metadata off each one, then demands 401 on every route with no token and with a valid TENANT token. A hand-written list would go stale the moment a route was added, which is the exact failure DEBT-066 describes. A controller must be registered in `PlatformModule` to be routed at all, so the enumeration cannot miss one.
- **Status:** Done
- **Handoff notes:** `pnpm turbo run lint typecheck test` green (13/13); **482 e2e across 21 suites** green, 22 of them new. Four things Fase 5 and Fase 6 need to know. (1) **An impersonation token cannot be revoked before it expires.** `POST /platform/impersonation/end` stamps `endedAt` and stops the audit record, but the JWT stays valid for the rest of its 30 minutes — a revocation list would mean a database read on every tenant request. Thirty minutes read-only is the bound that makes that acceptable; the frontend must therefore treat "ended" as a UI state and discard the token, not assume the server will start refusing it. (2) **`resetDatabase` does not delete `business_profiles`** (it is a per-tenant singleton every suite upserts), so any suite that provisions its own tenants must delete their profiles before the tenant rows or the delete fails its FK. Noted in that helper's doc comment now. (3) **Access-token revocation is not assertable over HTTP in a fast test** — `iat` has whole-second resolution, so a token minted in the same second as a logout legitimately survives it (the caveat already documented on `JwtAuthGuard`). The e2e suite asserts the refresh path and the `tokenValidFrom` row instead; the second-level behaviour is covered by unit tests. (4) `tenant-isolation.e2e-spec.ts` (Fase 6) is still missing — `platform.e2e-spec.ts` proves two tenants can hold a same-named product and that a new tenant gets its own system refs, but it does not sweep every list/detail/report endpoint the way ADR-025 Decision 3(c) requires.

### TASK-125 — v2 Phase 3: platform admin authentication

- **Date:** 2026-08-30
- **Module / Phase:** v2 Phase 3 — `PlatformAuthGuard`, the platform key pair, and the bootstrap CLI (plan `docs/plannings-v2/2026-08-29-multi-tenant-foundation.md` §Fase 3)
- **Objective:** Give the platform console a session that is cryptographically separate from every tenant session, and a way to create the first operator account that does not require an unauthenticated endpoint.
- **Relevant docs:** ADR-025 (Decision 5, §3.1–§3.4), ADR-011 §3 (the `tokenValidFrom` revocation pattern this mirrors), plan §3.1–§3.4.
- **What was done:** `PLATFORM_JWT_SECRET` and `PLATFORM_JWT_REFRESH_SECRET` added to `EnvSchema` in `app.module.ts` as required `min(32)` strings, and to `.env`, `.env.example`, `.env.test`, `.env.test.example` and both `env:` blocks of `.github/workflows/ci.yml`. `cookie.constants.ts` gained `PLATFORM_ACCESS_TOKEN_COOKIE` / `PLATFORM_REFRESH_TOKEN_COOKIE` and their max-ages. New `common/types/platform-jwt-payload.interface.ts` (`{ sub, email }` — no `role` claim). New `common/guards/platform-auth.guard.ts`: signature → row exists → `isActive` → `tokenValidFrom` with the same 1000ms `iat`-resolution allowance as `JwtAuthGuard`, then `request.platformAdmin` and `scope.platformAdminId`. New `PLATFORM_ACCOUNT_DEACTIVATED` message. New `scripts/create-platform-admin.ts`, registered as `pnpm create:platform-admin`. 11 unit tests in `platform-auth.guard.spec.ts`.
- **Decisions made during this task:** (1) **The guard lives in `common/guards/`, not in the platform module**, next to the four guards already there — this codebase keeps every guard in one directory and the plan's "module baru `modules/platform/`" is about the controllers and services Fase 4 adds. (2) **The platform session is deliberately shorter than a tenant session** — 2 hours access, 7 days refresh, against the tenant's 1 day / 30 days. One unattended browser here reaches every tenant in the system, which is not comparable to one shop's till. (3) **`scope.tenantId` stays `null`** (plan §3.3): platform code that reaches a tenant-scoped model through the filtered `PrismaService` throws `TenantContextMissingError` rather than reading whichever tenant was last in scope. (4) **The bootstrap CLI demands a 12-character password**, not the 8 `create-owner.ts` accepts, and never opens a tenant scope — a platform admin belongs to no tenant, so it only ever touches `UnscopedPrismaService`. (5) **The payload type has no `role`** — every platform admin has identical powers, and a privilege split inside the console would be its own ADR, not an implementation detail smuggled into a claim.
- **One plan step turned out to be a no-op:** §3.1 says to update `docker-compose.yml`. It never declared `JWT_SECRET` either — the `api` service sets only `NODE_ENV`, `PORT`, `DATABASE_URL` and `CORS_ORIGIN`, and every secret reaches the container through the bind-mounted `apps/api/.env`. Nothing to add; noted here so the next reader does not go looking for it.
- **Status:** Done
- **Handoff notes:** `pnpm turbo run lint typecheck test` green (13/13), 228 unit tests, and the 20 existing e2e suites still green at 460 tests. `create:platform-admin` was smoke-tested for real against `ohmypos_e2e` (create, duplicate-email rejection, short-password rejection) and the row removed afterwards; `ohmypos_db` still holds only the seeded `platform@ohmypos.local`. **The guard has no route to protect yet** — Fase 4 adds them, and until `platform-auth.e2e-spec.ts` (Fase 6) exists there is nothing that catches a `@Public()` platform controller that forgets `@UseGuards(PlatformAuthGuard)`. That spec should enumerate routes from the Nest router rather than list them by hand, or it will drift the moment a route is added. Two smaller things Fase 4 needs: there is deliberately **no `@ReqPlatformAdmin()` decorator yet** (nothing reads `request.platformAdmin` until a controller exists), and `TenantStatusGuard.isLogout` matches any path ending in `/auth/logout`, which will also match `POST /platform/auth/logout` — harmless, because that request carries no `request.user`, but worth knowing before anyone tightens it.

### TASK-124 — v2 Phase 2: tenant context, the Prisma filter, and the raw-SQL predicates

- **Date:** 2026-08-29
- **Module / Phase:** v2 Phase 2 — tenant resolution and enforcement (plan `docs/plannings-v2/2026-08-29-multi-tenant-foundation.md` §Fase 2)
- **Objective:** Make every query in `apps/api` tenant-scoped without threading a tenant argument through 25 modules, and make an unscoped query impossible rather than merely unlikely.
- **Relevant docs:** ADR-025 (Decisions 1, 2, 6), ADR-011 §3/§4 (unchanged guards), ADR-016 (lock ordering, unchanged), plan §2.1–§2.8.
- **What was done:** `common/prisma/tenant-context.ts` (an `AsyncLocalStorage` of `{ tenantId, platformAdminId }` plus `currentTenantId` / `requireTenantId` / `runWithTenant` / `enterTenantScope` / `adoptTenantScope`); `common/middleware/tenant-scope.middleware.ts` opens the scope for every request; `JwtAuthGuard` fills it from the `tenantId` it now selects on the round-trip it already made, and fails closed if the middleware did not run; `common/prisma/tenant.extension.ts` holds the whole filter as one `$allModels` rule; `PrismaModule` provides the extended client under the existing `PrismaService` token and the raw one as `UnscopedPrismaService`. `TenantStatusGuard` (403 on a `SUSPENDED` tenant, `POST /auth/logout` exempt) registered after `JwtAuthGuard`. `system-refs.ts` resolves both system categories by `(tenantId, name)`. `report-filters.ts` gained `tenantScope(alias)` and both shared fragments now lead with a bound `tenant_id`; `cashBalance` and the three polymorphic `LEFT JOIN sales` conditions were fixed by hand. `AuthService` takes `UnscopedPrismaService` for the two `@Public()` lookups and calls `adoptTenantScope` the moment it knows the user. Seed, `seed-volume.ts` and `scripts/create-owner.ts` bootstrap a tenant and enter its scope. 17 unit tests for the extension, 5 more for the SQL fragments.
- **Decisions made during this task:** (1) **`tenantId` carries `@default("")` in `schema.prisma`.** Without it Prisma types `tenantId` as required in every create input and the build breaks in 280 places — the plan's "no service changes" premise holds at runtime but not at compile time. The empty string is not a valid tenant id and no row holds it: an escaped write dies on `<table>_tenant_id_fkey`. (2) **The extension recurses into nested relation writes.** Discovered by `recipe_items_tenant_id_fkey` firing in two e2e suites: `product.create({ data: { recipeItems: { create: [...] } } })` stamped the parent and not the children. Relations are detected structurally (a plain object carrying `create`/`createMany`/`connectOrCreate`/`upsert`), which is safe here because no column in this schema is a JSON object. (3) **e2e suites get a tenant-BOUND client, not a scoped one** — Jest calls `beforeAll` and each `it` from sibling async contexts, so a scope entered in a hook is invisible in the test body. Verified with a probe spec, not assumed. `tenantBoundExtension(tenantId)` reuses the same `applyTenantScope` the API runs, so the tests still test what ships. (4) **`resetDatabase` does not delete `tenants`**, contrary to plan §6.2 — suites capture a tenant id in `beforeAll` and several call it again mid-run, so deleting the row underneath them turns every following insert into an FK error. `platform_admins` and `impersonation_sessions` are deleted. (5) `resolveLedgerBranchId` needed no change: TASK-120 had already moved it from a name lookup to `findFirst({ isSystem: true })`, which the extension narrows on its own — the plan was written against the older code.
- **Two real cross-tenant defects found while auditing the raw SQL** (plan §2.5 Group B), neither of which the extension could have caught: `matching.service.ts`'s `SELECT id FROM bank_transactions WHERE status = 'UNRESOLVED' … FOR UPDATE SKIP LOCKED` locked **every tenant's** unresolved rows, and `supplier-purchases.service.ts`'s latest-cost lookup scanned `supplier_purchase_items` across all tenants. Both now carry `tenant_id`. The remaining `FOR UPDATE` sites take ids that came from an already-filtered Prisma query, but the three whose id comes straight from the request body (`allocation` ×2, `payables` ×1) also gained the predicate — it changes no lock ordering (ADR-016 is by id) and removes the last way one tenant could take a lock on another's row.
- **Status:** Done
- **Handoff notes:** `pnpm turbo run lint typecheck test` green (13/13); 217 unit and **460 e2e across all 20 suites** green. Phase 3 (platform admin auth) has not started. Three things it must know: (1) `PLATFORM_MODELS` in `tenant.extension.ts` is the skip-list — `platform/*` services must inject `UnscopedPrismaService`, and `PlatformAuthGuard` must leave `scope.tenantId` null so an accidental touch of a tenant-scoped model throws instead of reading across tenants. (2) `@Public()` without `PlatformAuthGuard` is an open endpoint; `platform-auth.e2e-spec.ts` is the only thing that will catch it. (3) `tenantFixture()` in `test/tenant-fixture.ts` is where a second tenant gets built for `tenant-isolation.e2e-spec.ts` — it already returns the unscoped client for exactly that.

### TASK-123 — v2 Phase 1: the `tenantId` column, 35 composite foreign keys, and the backfill

- **Date:** 2026-08-29
- **Module / Phase:** v2 Phase 1 — schema and migration (plan §Fase 1). Approved explicitly before any schema edit, per AGENTS.md §AI Gatekeeper.
- **Objective:** Give all 23 tenant-scoped models a `tenantId`, add `Tenant` / `PlatformAdmin` / `ImpersonationSession`, and make a cross-tenant row reference impossible at the database level — on a live database, without losing data.
- **Relevant docs:** ADR-025 (Decisions 1, 5, 6, 8), ERD §3a, plan §1.1–§1.8.
- **What was done:** `schema.prisma` — `TenantStatus`, three new models, `tenantId` + `tenant` relation on all 23 models, five `@unique` name constraints and three `@unique([idempotencyKey])` turned composite, four tenant-prefixed hot-path indexes. One hand-written migration `20260829090000_add_multi_tenancy` in the plan's nine steps: platform tables → adopt the existing `BusinessProfile.name` as the default tenant → nullable column → backfill → `NOT NULL` → single-column FKs → swap the uniques → 13 composite `(id, tenant_id)` uniques and 35 composite FKs → indexes. Seed rewritten to bootstrap the tenant and a `PlatformAdmin` before entering the tenant scope.
- **Decisions made during this task:** (1) The migration was generated with `prisma migrate diff` against a shadow database and then reordered by hand — `prisma migrate dev --create-only` is interactive and refuses to run in this environment, and its own ordering adds `NOT NULL` before the backfill. (2) The 35 composite FKs are `DEFERRABLE INITIALLY DEFERRED` exactly as planned; the existing single-column FKs keep their `CASCADE`/`SET NULL` and the composites take `NO ACTION`, because mirroring `SET NULL` onto a composite containing the `NOT NULL` `tenant_id` cannot work. (3) `users.email` and `devices.activation_code` stay globally unique (ADR-025 Decision 6) — this is what keeps `AuthService.login` a plain lookup and the login page free of a tenant selector.
- **The plan was wrong about one thing, and it matters:** it states that Prisma ignores database constraints it does not know about. It does not. `prisma migrate diff` emits `DROP CONSTRAINT` for all 35 composite FKs and all 13 composite uniques, so **every future migration generated by Prisma will try to delete them** unless the generated SQL is edited. Logged as DEBT-063 with the exact check.
- **Status:** Done
- **Handoff notes:** Applied to `ohmypos_db` and `ohmypos_e2e`; a `pg_dump` of the pre-migration dev database was taken first. Verified on a clean shadow database rebuilt from zero: 1 tenant, no nullable `tenant_id` column anywhere, 35 composite FKs, 13 composite uniques, all 3 triggers still present, and `migrate diff` reporting *only* the 48 known composite-constraint drops. `ALTER COLUMN … SET DEFAULT ''` is part of the migration — see TASK-124 decision 1 for why, and do not remove it without also making `tenantId` required in every create call in `apps/api`. **Not started:** Phase 3 onward (`PlatformAdmin` auth, `/platform/*`, the `(platform)` route group, the isolation/impersonation e2e suites).
...
- **Date:** 2026-08-29
- **Module / Phase:** Branch module (`apps/api/src/modules/branches`), `common/system-refs.ts`, production bootstrap script, branch/user/POS/expense frontends
- **Objective:** Owner asked for "toko pertama otomatis jadi pusatnya, toko berikutnya jadi cabang". Investigating it surfaced two live defects on the same path (ERR-037, ERR-038), so the work shipped as three items in one migration.
- **Relevant docs:** ADR-014 (amended — see below), ADR-004, ADR-011, ADR-023; plan `docs/plannings/2026-08-29-store-identity-and-system-branch-protection.md`
- **What was done:**
  - **Migration `20260828201617_add_branch_system_and_main_store`** — added `branches.is_system` and `branches.is_main_store`, renamed the ADR-014 row from `Pusat (Dapur Sentral)` to **`Umum`**, backfilled the oldest non-system branch as main store, and added two partial unique indexes (`branches_single_system`, `branches_single_main_store`) so "exactly one" is a database guarantee rather than application logic. It also carries one pre-existing drift statement, see Handoff notes.
  - **`resolveLedgerBranchId` now finds the row by `isSystem`, not by name** (`system-refs.ts`). The same swap was applied to the two rejection sites, `sales.service.ts:104` and `supplier-purchases.service.ts:117`.
  - **`ensureSystemRefs(tx)`** — new single writer for the system branch and the two system categories, called by `create-owner.ts` (ERR-037), `seed.ts` and `seed-volume.ts`.
  - **Branch guards** — `SystemBranchProtectedException` on update/delete/set-main-store; `MainStoreProtectedException` on deleting the main store while other stores remain.
  - **`PATCH /branches/:id/main-store`** (`@Roles('OWNER')`), moving the designation in one transaction.
  - **Frontend** — the system row is filtered out of the Cabang page and both user dialogs (ERR-038); `PosScreen` and `BranchProfitabilityCard` switched from name matching to `!isSystem`; expense location label changed `Pusat` → `Umum` in six display sites; `BranchesTable` gained a Status column (Toko Utama / Cabang) and a "Jadikan Toko Utama" action with confirmation.
  - **Follow-up rename (same day, ERR-039):** the first label, `Umum (Semua Cabang)`, collided with this product's "no filter" sentinel `Semua Cabang` and sat next to it in the P&L filter. Migration `20260828211500_rename_system_branch_label` shortened it to **`Umum`**, which is also what every display surface already rendered. Data-only — possible precisely because `isSystem` had already taken the name out of the lookup path.
  - **Tests** — `test/branch-identity.e2e-spec.ts` (12), `test/bootstrap.e2e-spec.ts` (3), `BranchesTable.test.tsx` (2), `BranchesClient.test.tsx` (1), one added `CreateUserDialog` case, one added `ReportFilterBar` case pinning ERR-039.
- **Decisions made during this task:**
  1. **Renamed rather than merely hidden.** The Owner's diagnosis was that the NAME misleads — `Pusat (Dapur Sentral)` promises a physical flagship store, and their business has no central kitchen at all. The row is a *scope*, so it is now named like one.
  2. **`isSystem` column rather than renaming the constant.** Ganti-nama saja would have left the name as the lookup key, and would have **silently broken** `BranchProfitabilityCard.tsx:31`, which filtered with `.toLowerCase().includes('pusat')` — a filter that stops matching the moment the row is called `Umum`, letting the ledger scope into the per-branch profitability card.
  3. **Rejected the onboarding wizard the Owner first proposed** ("first branch = create your central store"). It would create a *sellable* store named as the central kitchen while the real system row already exists — two "pusat" with different meanings, institutionalised. With the system row provisioned automatically, the Owner's follow-up worry ("what if they create a cabang first, then the sentral") disappears: they never create it.
  4. **The main-store designation is set automatically, never via a switch.** A switch that is permanently disabled after the first store explains nothing, and moving the designation needs a separate action anyway.
  5. **Delete-guard order:** assigned-staff check runs *before* the main-store check. Found by `auth-rbac.e2e-spec.ts:779` failing — the main store is usually the one with staff, and "reassign these people first" is the more actionable of the two errors.
  6. `Account` provisioning was deliberately left out of `ensureSystemRefs` — an account's name and opening balance are business decisions, and an empty dropdown is visible on screen rather than a silent 503.
- **Status:** Done.
- **Handoff notes:**
  - **The migration contains one statement unrelated to this task:** `ALTER TABLE "supplier_purchase_items" ALTER COLUMN "conversion_factor" DROP DEFAULT`. This is pre-existing drift — the ADR-024 migration added the column *with* a default so it could backfill, and never dropped it, while `schema.prisma:658` declares no default. Prisma emits it on the next migration whatever that migration is about. It was kept rather than suppressed: the column is a per-line snapshot, and a silent default of `1` would mask a write path that forgot to record the real conversion factor. Nothing in the current code relies on the default (all e2e purchase suites pass).
  - **`ensureSystemRefs` must never become an `upsert` keyed on the branch name.** If the row has been relabelled, upsert-by-name inserts a *second* system row and `branches_single_system` rejects it. `bootstrap.e2e-spec.ts` case 3 pins this, and was verified to fail against the upsert form.
  - **New branch pickers must filter `!branch.isSystem`.** All three former name comparisons are gone; do not reintroduce one.
  - `isMainStore` currently carries **no functional meaning** — it is a label. It is deliberately not wired into POS defaults, expense defaults, or report ordering.
  - E2E specs that create branches through `prisma.branch.create` bypass the main-store rule in `BranchesService.create`. `branch-identity.e2e-spec.ts` goes through `POST /branches` for exactly this reason.
  - No commit or push was performed.

### TASK-119 — v2 foundation: multi-tenancy decision and planning documents

- **Date:** 2026-08-29
- **Module / Phase:** v2 Phase 0 — documentation only, no code
- **Objective:** Decide how OhMyPos becomes multi-tenant and gains a super-admin dashboard, and record that decision plus an executable plan before any schema or code changes.
- **Relevant docs:** ADR-025 (written by this task), ADR-011 (partially superseded), PRD §3 Non-goals, System Design §5/§8, ERD §3a, `docs/plannings-v2/2026-08-29-multi-tenant-foundation.md`.
- **What was done:** Audited the current schema, API auth layer, and frontend for what multi-tenancy would actually touch. Wrote **ADR-025** (shared DB + `tenantId`, separate `PlatformAdmin` identity, no RLS, three enforcement layers). Marked ADR-011 "partially superseded" and bumped the ADR doc to Draft v3. Wrote `docs/plannings-v2/2026-08-29-multi-tenant-foundation.md` — a six-phase executable plan including the literal SQL for 13 composite uniques and 35 composite foreign keys. Updated PRD §3 (multi-tenant moved from non-goal to v2 scope), System Design §5 (the `(platform)` route group) and §8 (a new "Tenant scoping" subsection), and ERD §3a (`Tenant`, `PlatformAdmin`, `ImpersonationSession`, and the `tenantId` column). No source file was touched.
- **Decisions made during this task:** Four that ADR-025 records in full, plus three smaller ones. (1) `tenantId` goes on **all** 23 models including child tables — the redundancy is what lets the Prisma filter be a single `$allModels` rule with no special cases. (2) Composite FKs are written as raw SQL in the migration rather than modeled in Prisma, because Prisma is awkward about one column participating in many relations; Prisma tolerates DB constraints it does not know about. (3) Those composite FKs are `DEFERRABLE INITIALLY DEFERRED` — mirroring the existing `ON DELETE SET NULL` actions onto them would try to null a `NOT NULL` `tenant_id`, and checking them per-statement would fire mid-cascade. (4) `Tenant.id` is `String @default(uuid())` mapping to `TEXT`, not `@db.Uuid` — no column in this schema uses `@db.Uuid`, and an initial draft of the ADR got this wrong before the migration SQL was checked against `20260814191153_init`.
- **Status:** Done
- **Handoff notes:** **Phase 1 has not started and must not start without explicit approval** — it changes `schema.prisma` and creates a migration (AGENTS.md §AI Gatekeeper). The three things that will bite whoever executes it, in order of how easy they are to miss: (1) `apps/api/src/common/system-refs.ts` resolves the central branch and both system categories **by globally-unique name** — leave it alone and tenant B's sales attach to tenant A's "Pusat (Dapur Sentral)" branch with no error raised; the composite FKs will catch it at COMMIT, but do not rely on that. (2) The entire `reports.service.ts` is raw SQL and invisible to the Prisma extension — eight call sites need hand-written `tenant_id` predicates, and only `tenant-isolation.e2e-spec.ts` will ever catch a missed one. (3) `prisma/seed.ts` drives real services rather than Prisma directly (so denormalized balances keep one writer), so it breaks the moment those services become tenant-aware; the fix is to wrap the seed body in `tenantStorage.run(...)` rather than to thread a tenant argument through it. Also note `apps/web` has no `middleware.ts` — Next 16 names it `proxy.ts`. Two globally-unique constraints stay global **on purpose** (`users.email`, `devices.activation_code`); do not "fix" them for consistency.
...
- **What was done:**
  ...
  - **Matching double-proposal race (DEF-QA-03):** `MatchingService.proposeMatches()` now runs inside `this.prisma.$transaction(..., { maxWait: 5000, timeout: 15000 })`, selecting `UNRESOLVED` bank transactions via raw `SELECT id ... FOR UPDATE SKIP LOCKED` before reading/matching/updating them — two concurrent propose calls can no longer both claim the same bank transaction.
  - **Frontend controlled Select (DEF-QA-04):** `SplitAllocationDialog.tsx`'s ledger-entry `Select` changed from `value={line.ledgerEntryId || undefined}` to `value={line.ledgerEntryId}` — the `|| undefined` toggled the component between uncontrolled (empty draft row) and controlled (once picked), which is the exact transition React warns about. `AccountFormDialog.tsx` (the plan's other named file) was checked and found already correct — no change made.
  - **Timezone cutoff SQL (DEF-004, investigated, not changed):** `reports.service.ts`'s cash-balance cutoff compares `le.entry_date` (a naive `TIMESTAMP(3)`) against a `Date` from `resolveReportRange`/`wibMidnight` — both sides are UTC-instant `Date` objects serialized the same way by the same Prisma raw-query path, so the comparison is already internally consistent. The plan's proposed `AT TIME ZONE 'UTC'` cast was not applied since it would not fix anything real and risked coupling the comparison to the DB session's `timezone` GUC.
  - **Test coverage added:** `ledger-entries/:id` RBAC/IDOR cases in `auth-rbac.e2e-spec.ts`; a concurrent create/revoke regression and a concurrent double-propose regression, both in `allocation-sum.e2e-spec.ts`; a dedicated backdate-limit describe block (KASIR rejected past 3 days, OWNER/ADMIN exempt) in both `sales.e2e-spec.ts` and `purchasing-payables.e2e-spec.ts` — TASK-087's `BACKDATE_LIMIT_DAYS` rule had shipped with zero test coverage anywhere in the repo.
- **Decisions made during this task:** (1) `revoke()` re-reads the allocation unlocked first purely to learn its `bankTransactionId`/`ledgerEntryId` before locking the real rows — the second, locked re-read is what the NotFound/already-revoked checks run against, so this can't be short-circuited. (2) The DEF-004 timezone item was investigated rather than blindly patched, per the adversarial review's own "never fabricate, verify before applying" standard — a change that doesn't fix a real bug is not a safer default. (3) Backdate-limit tests reuse each file's existing dedicated fixture (`pGuardId` in sales, `rawMaterialGulaId` in purchasing) rather than adding new ones, keeping the stock-integrity check (`sales.e2e-spec.ts` Case 23) consistent.
- **Status:** Done
- **Handoff notes:** `pnpm exec turbo run lint typecheck` green (9/9). Backend: 18 e2e suites / 414 tests, 22 unit suites / 166 tests, all green (`--runInBand`, real Postgres). Frontend: 59 suites / 433 tests green. No schema or migration changes, no new dependencies — everything was a controller/service/test change within existing files. The uncommitted test edits already in the working tree at review time (`kasir` → `owner` cookie swaps in `monthly-cycle.e2e-spec.ts`/`inventory.e2e-spec.ts`, `new Date().toISOString()` in place of fixed 2026-08-16 dates in `sales.e2e-spec.ts`/`purchasing-payables.e2e-spec.ts`) were verified as legitimate consequences of TASK-087's backdate limit, not test-weakening — left as-is.
