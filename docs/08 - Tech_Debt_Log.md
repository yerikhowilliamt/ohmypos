...
- **Priority:** Medium
- **Status:** **Resolved 2026-08-23 (TASK-081)** — applied exactly as proposed, with `PayablesTab` as the reference. Both hooks now take a params object and share one `build*Query` with their `fetch*Page`, so the exported set cannot drift from the screen; both tables render the footer at `DEFAULT_EXPENSES_PAGE_SIZE = 10`, matching the Payables tab beside them. **One thing the proposal did not mention and that mattered more than the footer:** five columns across the two tables carried a `SortableHeader` the backend has no key for (`note`, `sourceType`, `supplierName`, `branchId`, `paymentStatus`). Under server pagination those would have reordered the visible 10 rows while looking like they ordered the whole ledger — the same lying-control class the footer was added to remove. They now render plain headers, following `StockMovementsTable`. Covered by 14 new web tests (7 per tab), sabotage-verified three ways: dropping the `pagination` prop, rebuilding the export params independently, and restoring a fake sortable header each redden only their own cases. The export/table row mismatch TASK-073 left behind is gone.
...
- **Update 2026-08-23 (TASK-081):** this now covers **three** tabs, not one. The parenthetical above said `PurchaseEntryTab` was "not comparable" because it had neither search nor server pagination — TASK-081 gave it and `GeneralExpenseTab` server pagination, so all three tabs of the Pengeluaran screen are now server-paginated with no search box. The gap also got sharper rather than milder: both tabs previously showed 50 rows at once and now show 10, so paging by hand to find one entry is five times the work it was. `Pengeluaran Umum` is the worst of the three — it holds every OUTFLOW, manual and generated, so it is the longest list on the screen. Server-side plumbing for it already exists: `LedgerEntryQuerySchema` and `SupplierPurchaseQuerySchema` both now take the full `page`/`limit`/`sortBy`/`sortOrder` set (TASK-074), so only `search` is missing.
- **Why deferred:** Adding a search box to a screen that has none is a new feature, not a defect fix, and TASK-072's scope was the boxes that lied about what they searched (AGENTS.md §Strict Scope). The same reasoning kept it out of TASK-081, whose scope was the missing pagination footer. The plumbing it would need already exists and is proven: `search` on the query schema, an `OR` in the service, `serverSearch` on `DataTable`, `useDebouncedValue` in the client.
- **Impact if unaddressed:** Someone chasing one supplier's unpaid invoice pages through the list or narrows by supplier dropdown first. Nothing is wrong on screen; the workflow is just slower than the four screens beside it — and since TASK-081, slower on all three tabs of that screen.
- **Trigger condition:** The first request to "find this invoice/supplier/expense" from the Pengeluaran screen, or when any of the three lists routinely runs past a couple of pages. `Pengeluaran Umum` will reach that first.
- **Proposed resolution:** Copy the TASK-072 pattern exactly — `search: z.string().trim().optional()` on `PayableQuerySchema`, `LedgerEntryQuerySchema` and `SupplierPurchaseQuerySchema`; an `OR` in each service (supplier name for payables and purchases, `note` for ledger entries); `serverSearch` + `useDebouncedValue` + reset-to-page-1 at the three call sites. Worth doing as one task across all three, for the same reason TASK-072 took four modules rather than two: they share `DataTable` and the same client pattern.
- **Priority:** Low
- **Status:** Open
...
- **Priority:** Low
- **Status:** **Resolved 2026-08-23 (TASK-074)** — applied per module exactly as proposed, and `sortOrder` deliberately **stayed off** `PaginationQuerySchema`. One correction to the proposed resolution worth carrying forward: the default is not `'desc'` everywhere. `SuppliersService` ordered by `name` ascending, so `sortOrder = 'asc'` is its default — copying `'desc'` there would have silently reversed the master-data supplier list and every supplier dropdown for callers that pass nothing. Covered by 11 e2e cases (7 in `purchasing-payables.e2e-spec.ts`, 4 in `reconciliation-addendum.e2e-spec.ts`), each fenced by a date window or `search` prefix unique to its block and with the date order deliberately disagreeing with the amount order, so `sortBy` and `sortOrder` are observable independently. Sabotage-verified: restoring the three hardcoded literals reddens exactly 5 cases. This closes the TASK-067…074 series.
