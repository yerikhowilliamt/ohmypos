...
- PDF bank statement parsing is supported for the Mandiri Livin e-statement only (ADR-022, reversing the original PRD §10 deferral). Other issuers are still CSV-only, and password-protected PDFs are rejected rather than decrypted — `pdf-parse` has no password support. The Mandiri PDF parser reads the table by column x-position, not by line regex; re-derive the geometry against a real sample before changing it.
